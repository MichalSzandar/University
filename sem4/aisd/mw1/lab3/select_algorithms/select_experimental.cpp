...
int medianOfMedians(int arr[], int left, int right) {
    int n = right - left + 1;
    int numMedians = (n + size_of_groups - 1) / size_of_groups;
    int* medians = new int[numMedians];

    for (int i = 0; i < numMedians; ++i) {
        int subLeft = left + i * size_of_groups;
        int subRight = std::min(subLeft + size_of_groups - 1, right);
...
int main(int argc, char *argv[]) {
    if (argc < 2) {
        std::cerr << "Usage: " << argv[0] << " <size_of_groups>" << std::endl;
        return 1;
    }
    
    size_of_groups = std::stoi(argv[1]);
